...
| B-D4 | music-search = pfplay-web 컴포넌트 **포팅** (필드명 상이 — **boundary 매퍼 필수**) | AddPackTrackRequest는 `name/linkId/duration/thumbnailImage`, web 검색 DTO는 `videoTitle/videoId/runningTime/thumbnailUrl`. 어휘가 달라 spread 금지·명시 매퍼로 매핑(§4.3) |
...
### 3.6 `GET /api/v1/admin/virtual-dj/music-search?q=` — 어드민 검색 프록시 (C1 해소)
- **필수**: 기존 `MusicSearchController`(`/api/v1/music-search`)는 `@PreAuthorize("hasRole('MEMBER')")` =
  **회원 전용**이라 AdminAccessToken(ROLE_ADMIN)으로는 **403**. → 어드민 스코프 검색 엔드포인트를 신설한다.
- `@adminAuth.canManageVirtualDj()` 게이팅, 기존 `MusicSearchService.getSearchList(q)` 그대로 위임,
  응답은 기존 `QueryMusicSearchResponse` 재사용. 송팩 빌더가 이걸 호출.
- 모듈: `MusicSearchService`는 `playlist` 모듈, 어드민 엔드포인트는 `virtualdj`(app 모듈). app→playlist 의존
  가시성 확인(plan task). 불가 시 playlist 모듈에 어드민 검색 컨트롤러를 두고 `canManageVirtualDj` 게이팅.

### 3.7 테스트
컨트롤러 슬라이스 + 쿼리 통합 테스트. 권한 게이팅(`canManageVirtualDj`) 검증. 검색 프록시는 회원 전용
엔드포인트와 **별개 인증 경로**임을 테스트로 잠금.
...
- music-search 포팅: pfplay-web `features/playlist/add-tracks`의 검색박스→결과리스트→선택 흐름을
  admin 스택(Vite + 기존 http 클라이언트)으로 이식. **검색 응답 → AddPackTrackRequest boundary 매퍼**
  (`videoTitle→name`, `videoId→linkId`, `runningTime→duration`, `thumbnailUrl→thumbnailImage`) —
  spread 금지, 명시 매퍼 함수 1개로(어휘 상이, [[reference_dto_vocabulary_zero_content_hidden]] 교훈).
...
## 7. plan 단계 코드확인 항목 — **모두 해소(2026-06-01)**

- **C1 (검색 인증) → 해소**: `MusicSearchController`(`/api/v1/music-search`)는 `@PreAuthorize("hasRole('MEMBER')")`
  = 회원 전용. AdminAccessToken으로는 403. **→ 어드민 검색 프록시 신설(§3.6)**. 단순 deferrable 아니라 실제 백엔드 task.
- **C2 (role 매핑) → 해소**: `canManageVirtualDj() → isAdmin() = ROLE_ADMIN`
  (`common/.../AdminAuthorizationSpEL.java`). nav role 게이팅 = ADMIN(§4.1).
- **C3 (풀 요약 쿼리) → 해소**: `BotPoolQueryRepositoryImpl`은 QueryDSL + NOT EXISTS(active crew) 패턴.
  count(봇)/count(idle)/group-by(crew→partyroom) 메서드 추가로 동일 패턴 확장. cross-BC는 adapter 내부 한정.
- **C4 (파티룸 목록 조인) → 해소**: `AdminPartyroomQueryRepositoryImpl.findAdminList`는 QueryDSL tuple projection
  + correlated subquery(`djCountSubquery`). **봇 DJ 수 correlated subquery + `PartyroomVirtualDjConfigData` left join**
  추가 + `AdminPartyroomListRow`/`AdminPartyroomListItemResponse`에 nullable `virtualDj` 필드 추가로 구현.
...
- **pfplay-platform**: GET 3종(풀 요약 / 송팩 목록 / 송팩 상세) + **어드민 검색 프록시(§3.6)** +
  어드민 파티룸 목록 DTO 봇 요약 조인 + 테스트.
