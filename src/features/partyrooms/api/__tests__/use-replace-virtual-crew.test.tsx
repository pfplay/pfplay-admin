...
describe("useReplaceVirtualCrew", () => {
  ...
  it("on success: invalidates ['virtual-crew','room',id] and ['partyrooms'], toast.success", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/partyrooms/5/virtual-crew/replace", () => new HttpResponse(null, { status: 204 })),
    )

    const { result } = renderHook(() => useReplaceVirtualCrew(5), { wrapper })
    ...
    expect(successSpy).toHaveBeenCalledWith("재배치 완료")
  })
  ...
  it("on 409 error: does NOT invalidate", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/partyrooms/5/virtual-crew/replace", () =>
        HttpResponse.json(
