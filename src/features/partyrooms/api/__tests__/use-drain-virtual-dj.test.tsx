...
describe("useDrainVirtualDj", () => {
  ...
  it("on success: invalidates ['virtual-dj','room',id] and ['partyrooms'], toast.success", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/partyrooms/5/virtual-dj/drain", () => new HttpResponse(null, { status: 204 })),
    )

    const { result } = renderHook(() => useDrainVirtualDj(5), { wrapper })
  ...
  it("on 409 error: does NOT invalidate", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/partyrooms/5/virtual-dj/drain", () =>
        HttpResponse.json(
