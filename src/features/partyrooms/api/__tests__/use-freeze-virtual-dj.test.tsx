...
describe("useFreezeVirtualDj", () => {
  ...
  it("on success: invalidates ['virtual-dj','room',id] and ['partyrooms'], toast.success", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/partyrooms/5/virtual-dj/freeze", () => new HttpResponse(null, { status: 204 })),
    )

    const { result } = renderHook(() => useFreezeVirtualDj(5), { wrapper })
    ...
    expect(successSpy).toHaveBeenCalledWith("봇 동결 완료")
  })
  ...
  it("on 409 error: does NOT invalidate", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/partyrooms/5/virtual-dj/freeze", () =>
        HttpResponse.json(
