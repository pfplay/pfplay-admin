...
describe("useDrainResourcesVirtualCrew", () => {
  ...
  it("on success: invalidates ['virtual-crew','room',id] and ['partyrooms'], toast.success", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/partyrooms/5/virtual-crew/drain-resources", () => new HttpResponse(null, { status: 204 })),
    )

    const { result } = renderHook(() => useDrainResourcesVirtualCrew(5), { wrapper })
    ...
    expect(successSpy).toHaveBeenCalledWith("봇 리소스 회수 완료")
  })
  ...
  it("on 409 error: does NOT invalidate", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/partyrooms/5/virtual-crew/drain-resources", () =>
        HttpResponse.json(
