...
import { VirtualCrewBulkDialog } from "../virtual-crew-bulk-dialog"
...
describe("VirtualCrewBulkDialog", () => {
  ...
  it("OFF (기본) → target/floor 숨김, 송팩 숨김", () => {
    ...
    expect(screen.queryByLabelText("DJ 봇 수")).not.toBeInTheDocument()
  ...
  it("OFF submit → target/floor/songPack null 전송", async () => {
    ...
    renderWithClient(
      <VirtualCrewBulkDialog {...baseProps} onSuccess={onSuccess} />,
    )
    fireEvent.click(screen.getByRole("button", { name: "일괄 적용" }))
