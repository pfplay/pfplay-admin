...
import type { SongPackDetail } from "@/entities/virtual-crew"
...
function mockDetail(detail: SongPackDetail) {
  server.use(
    http.get("*/api/v1/admin/virtual-crew/song-packs/:id", () =>
...
describe("SongPackBuilder", () => {
  ...
  it("music-search 결과 선택 → addTrack 에 toPackTrack 매핑된 바디 전송", async () => {
    ...
    server.use(
      http.post(
        "*/api/v1/admin/virtual-crew/song-packs/:id/tracks",
  ...
  it("트랙 제거 버튼 → DELETE trackId 호출", async () => {
    ...
    server.use(
      http.delete(
        "*/api/v1/admin/virtual-crew/song-packs/:id/tracks/:trackId",
        ({ params }) => {
