...
describe("song-packs-api", () => {
  ...
  it("getSongPack — GET 송팩 상세 unwrap (트랙 포함)", async () => {
    server.use(
      http.get("*/api/v1/admin/virtual-crew/song-packs/7", () =>
        HttpResponse.json({
          data: {
            id: 7,
            name: "재즈",
  ...
  it("createSongPack — POST body {name,description} → id 반환", async () => {
    ...
    server.use(
      http.post(
        "*/api/v1/admin/virtual-crew/song-packs",
        async ({ request }) => {
  ...
  it("renameSongPack — PUT body {name} (204)", async () => {
    ...
    server.use(
      http.put(
        "*/api/v1/admin/virtual-crew/song-packs/5",
        async ({ request }) => {
          bodySeen = await request.json()
  ...
  it("deleteSongPack — DELETE (204)", async () => {
    let called = false
    server.use(
      http.delete("*/api/v1/admin/virtual-crew/song-packs/9", () => {
        called = true
        return new HttpResponse(null, { status: 204 })
      }),
    )
  ...
  it("addTrack — POST .../tracks body → id 반환", async () => {
    ...
    server.use(
      http.post(
        "*/api/v1/admin/virtual-crew/song-packs/3/tracks",
        async ({ request }) => {
          bodySeen = await request.json()
  ...
  it("removeTrack — DELETE .../tracks/{trackId} (204)", async () => {
    ...
    server.use(
      http.delete(
        "*/api/v1/admin/virtual-crew/song-packs/3/tracks/100",
        () => {
          called = true
          return new HttpResponse(null, { status: 204 })
        },
  ...
  it("409 (이름 중복/사용중) → ApiError 전파", async () => {
    server.use(
      http.post("*/api/v1/admin/virtual-crew/song-packs", () =>
        HttpResponse.json(
          {
            status: 409,
  ...
  it("404 (미존재) → ApiError 전파", async () => {
    server.use(
      http.get("*/api/v1/admin/virtual-crew/song-packs/999", () =>
        HttpResponse.json(
