...
describe("personas-api", () => {
  ...
  it("getPersona — GET 페르소나 상세 unwrap (instruction 포함)", async () => {
    server.use(
      http.get("*/api/v1/admin/virtual-crew/personas/7", () =>
        HttpResponse.json({
          data: {
            id: 7,
            name: "재즈 DJ",
  ...
  it("createPersona — POST body {name,instruction} → id 반환", async () => {
    ...
    server.use(
      http.post("*/api/v1/admin/virtual-crew/personas", async ({ request }) => {
  ...
  it("updatePersona — PUT body {name,instruction,active} (204)", async () => {
    let bodySeen: unknown
    server.use(
      http.put("*/api/v1/admin/virtual-crew/personas/5", async ({ request }) => {
        bodySeen = await request.json()
  ...
  it("deletePersona — DELETE (204)", async () => {
    let called = false
    server.use(
      http.delete("*/api/v1/admin/virtual-crew/personas/9", () => {
        called = true
        return new HttpResponse(null, { status: 204 })
      }),
    )
  ...
  it("404 (미존재) → ApiError 전파", async () => {
    server.use(
      http.get("*/api/v1/admin/virtual-crew/personas/999", () =>
        HttpResponse.json(
